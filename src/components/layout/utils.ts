 ...
 * @param activeItem
 */
export const handleURLQueries = (params: Readonly<Params<string>>, location: Location, path: string | undefined): boolean => {
  if (Object.keys(params).length && path) {
    const arr = Object.keys(params)
    const fullpath = location.pathname+location.search

    return fullpath.includes(path) && fullpath.includes(params[arr[0]] as string)
  }
  return false
}
