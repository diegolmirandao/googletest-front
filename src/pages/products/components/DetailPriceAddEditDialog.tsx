...
interface IProps {
  open: boolean;
  loading: boolean;
  onSubmit: (formFields: IAddUpdateProductDetailPrice) => void;
  onClose: () => void;
}
 ...
 */
const ProductDetailPriceAddEditDialog = (props: IProps) => {
  // ** Props
  const { open, loading, onSubmit, onClose } = props;
  // ** Reducers
  const { productReducer: { currentProductDetailPrice }, currencyReducer: { currencies }, productPriceTypeReducer: { productPriceTypes } } = useAppSelector((state) => state);
  // ** Vars
  const defaultValues: IAddUpdateProductDetailPrice = {
    price_type_id: currentProductDetailPrice?.type.id ?? 1,
    currency_id: currentProductDetailPrice?.currency.id ?? 1,
    amount: currentProductDetailPrice?.amount ?? 0
  };
  ...
  const {
    ...
    getValues,
    formState: { errors }
  } = useForm<IAddUpdateProductDetailPrice>({
    defaultValues,
    mode: 'onChange'
  });
  ...
  return (
    ...
    >
      <DialogTitle sx={{ position: 'relative' }}>
        {currentProductDetailPrice ? t('edit_price') : t('add_price')}
        <IconButton
          size='small'
          onClick={handleClose}
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <DialogContent>
          <FormControl fullWidth sx={{ mb: 6 }}>
            ...
            />
            {errors.price_type_id && <FormHelperText sx={{ color: 'error.main' }}>{t(`${errors.price_type_id.message}`)}</FormHelperText>}
          </FormControl>
            ...
            <Controller
              ...
              render={({ field: { value, onChange } }) => (
                <CurrencyInput
                  ...
                  label={t('amount')}
                  onChange={onChange}
                  currency={currencies.find(currency => currency.id == getValues(`currency_id`))}
...
export default ProductDetailPriceAddEditDialog;
