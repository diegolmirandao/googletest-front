...
interface IProps {
  open: boolean;
  loading: boolean;
  onSubmit: (formFields: IAddUpdateBrand) => void;
  onClose: () => void;
}
 ...
 * @returns Brand Form Dialog component
 */
const BrandAddEditDialog = (props: IProps) => {
  // ** Props
  const { open, loading, onSubmit, onClose } = props;
  // ** Reducers
  const { brandReducer: { currentBrand } } = useAppSelector((state) => state);

  const defaultValues: IAddUpdateBrand = {
...
export default BrandAddEditDialog;
