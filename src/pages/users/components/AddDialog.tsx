...
import { addSchema } from 'src/schemes/user';
...
interface IProps {
  open: boolean;
  loading: boolean;
  onSubmit: (formFields: IAddUser) => void;
  onClose: () => void;
}
...
const UserAddDialog = (props: IProps) => {
  ...
  const defaultValues: IAddUser = {
    ...
    role_id: 1,
    status: true,
    permissions: roles[0].permissions.map(permission => permission.name)
  }
   ...
   */
  const groupPermissions = (roleId: number = watchRoleId) => {
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogContent>
            ...
            <FormControl fullWidth sx={{ mb: 6 }}>
              ...
              />
              {errors.password && <FormHelperText sx={{ color: 'error.main' }}>{ t(`${errors.password.message}`) }</FormHelperText>}
            </FormControl>
