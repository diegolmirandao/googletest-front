...
interface IProps {
  open: boolean;
  onEditClick: () => void;
  onSubcategoryAddClick: () => void;
  onSubcategoryEditClick: (subcategory: MProductSubcategory) => void;
  onSubcategoryDeleteClick: (subcategory: MProductSubcategory) => void;
  onDeleteClick: () => void;
  onClose: () => void;
}

/**
 * Product Category Details Dialog
 * @param props component parameters
 * @returns Product Category Detail Dialog component
 */
const ProductCategoryDetailDialog = (props: IProps) => {
  // ** Props
  const { open, onEditClick, onDeleteClick, onSubcategoryAddClick, onSubcategoryEditClick, onSubcategoryDeleteClick, onClose } = props;
  // ** Reducers
  const { productCategoryReducer: { currentProductCategory } } = useAppSelector((state) => state);
  // ** Vars
  ...
  const columns = [
    ...
    },
    {
      ...
      getActions: ({ row }: GridRowParams) => [
        <GridActionsCellItem icon={<Tooltip title={t('edit')}><PencilOutlineIcon /></Tooltip>} onClick={() => onSubcategoryEditClick(row)} label={t('edit')} />,
        <GridActionsCellItem icon={<Tooltip title={t('delete')}><DeleteOutlineIcon /></Tooltip>} onClick={() => onSubcategoryDeleteClick(row)} label={t('delete')} />
      ]
    }
  ];
          ...
          <Grid container spacing={6}>
              ...
              <Box>
                ...
                <Box sx={{ pt: 2, pb: 1 }}>
                  <Box sx={{ mb: 1 }}>
                    ...
                    <Typography variant='body1' sx={{ textAlign: 'right' }}>{currentProductCategory?.name}</Typography>
                  </Box>
                </Box>
              </Box>
              {/*
            ...
            <Grid item xs={12} md={6} lg={9} >
              <TabContext value={tabValue}>
                ...
                >
                  <Tab value='subcategories' label={t('subcategories')} icon={<FileTreeOutlineIcon sx={{ fontSize: '18px' }} />} />
                ...
                <Box sx={{ mt: 3 }}>
                    ...
                    */}
                    <Box sx={{ display: 'flex', justifyContent: 'right' }}>
                      <Box sx={{ display: {xs: 'none', md: 'inline-flex'}, mb: 2}}>
                          <Button sx={{ mr: 2 }} onClick={onSubcategoryAddClick} variant='outlined' color='primary' startIcon={<PlusIcon fontSize='small' />}>
                            {t('add')}
                          </Button>
                      </Box>
                      <Box sx={{ display: {xs: 'inline-flex', md: 'none'}, mr:4, my: 2 }}>
                          <Button sx={{ mr: 2 }} onClick={onSubcategoryAddClick} color='primary' variant='outlined'><PlusIcon fontSize='small'/></Button>
                      </Box>
                    </Box>
                    <DataGridPro
                      columns={columns} 
                      rows={currentProductCategory?.subcategories ?? []} 
                  ...
                  <TabPanel sx={{ p: 0 }} value='multimedia'>
                    {/*
                      START PRODUCT CATEGORY MULTIMEDIA
                    */}
                    {/*
                      END PRODUCT CATEGORY MULTIMEDIA
                    */}
                  </TabPanel>
                  <TabPanel sx={{ p: 0 }} value='parameters'>
                    {/*
                      START PRODUCT CATEGORY PARAMETES
                    */}
                    {/*
                      END PRODUCT CATEGORY PARAMETES
                    */}
                  </TabPanel>
                </Box>
              </TabContext>
...
export default ProductCategoryDetailDialog;
