...
interface IProps {
  open: boolean;
  loading: boolean;
  onSubmit: (formFields: IAddUpdateProductSubcategory) => void;
  onClose: () => void;
}
 ...
 * @returns ProductSubcategory Form Dialog component
 */
const ProductSubcategoryAddEditDialog = (props: IProps) => {
  // ** Props
  const { open, loading, onSubmit, onClose } = props;
  // ** Reducers
  const { productCategoryReducer: { currentProductSubcategory } } = useAppSelector((state) => state);

  const defaultValues: IAddUpdateProductSubcategory = {
    name: currentProductSubcategory?.name ?? ''
  }
    ...
    <>
      ...
      >
        <DialogTitle sx={{ position: 'relative' }}>
          {currentProductSubcategory ? t('edit_subcategory') : t('add_subcategory')}
          <IconButton
            size='small'
            onClick={handleClose}
...
export default ProductSubcategoryAddEditDialog;
