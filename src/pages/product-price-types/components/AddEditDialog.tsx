...
interface IProps {
  open: boolean;
  loading: boolean;
  onSubmit: (formFields: IAddUpdateProductPriceType) => void;
  onClose: () => void;
}

/**
 * Add and edit product price type form
 * @param props component parameters
 * @returns Product Price Type Form Dialog component
 */
const ProductPriceTypeAddEditDialog = (props: IProps) => {
  // ** Props
  const { open, loading, onSubmit, onClose } = props;
  // ** Reducers
  const { productPriceTypeReducer: { currentProductPriceType } } = useAppSelector((state) => state);

  const defaultValues: IAddUpdateProductPriceType = {
    name: currentProductPriceType?.name ?? ''
  }
    ...
    <>
      ...
      >
        <DialogTitle sx={{ position: 'relative' }}>
          {currentProductPriceType ? t('edit_price_type') : t('add_price_type')}
          <IconButton
            size='small'
            onClick={handleClose}
...
export default ProductPriceTypeAddEditDialog;
